...
*   **詳細分析と考察**
    ...
    * [ ] **ハイブリッドスコアの重み調整:** 現在は単純平均だが、AEスコアとSVMスコアの最適な重み付け（例: `w1*AE + w2*SVM`）を探索し、さらなる性能向上を目指す。
    * [ ] **SVMハイパーパラメータチューニング:** `GridSearchCV`などを用いて、SVMの `C` や `gamma` といったパラメータを最適化し、分類性能の向上を試みる。
* **4-3. 卒業論文の執筆**
